...
use super::constants::{
    ...
    KIMI_DIR, PI_CODING_AGENT_DIR_ENV, PI_DIR, PI_EXTENSIONS_SUBDIR, PI_LOCAL_DIR, PI_PLUGIN_FILE,
...
fn install_kimi_hooks(ctx: InitContext) -> Result<()> {
    ...
    if config_path.exists() {
        let raw = fs::read_to_string(&config_path)
            .with_context(|| format!("Failed to read Kimi config: {}", config_path.display()))?;
        ...
        if new_content.trim() != raw.trim() {
            if dry_run {
                ...
                fs::rename(&tmp_path, &config_path).with_context(|| {
                    format!("Failed to write Kimi config: {}", config_path.display())
                })?;
        ...
        if dry_run {
            ...
            let tmp_path = tmp.into_temp_path();
            fs::rename(&tmp_path, &config_path).with_context(|| {
                format!("Failed to create Kimi config: {}", config_path.display())
            })?;
...
mod tests {
    ...
    fn test_pi_global_uninstall_removes_plugin() {
        ...
        with_pi_dir_override(&tmp, |pi_dir| {
            ...
            uninstall(true, false, false, false, false, true, InitContext::default()).unwrap();
    ...
    fn test_pi_local_uninstall_removes_plugin() {
        ...
        let result = uninstall(false, false, false, false, false, true, InitContext::default());
        std::env::set_current_dir(&cwd).unwrap();
        result.unwrap();
