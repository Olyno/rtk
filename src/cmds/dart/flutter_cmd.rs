...
pub(crate) fn summarize_flutter_test(raw: &str) -> String {
    ...
    for line in &clean_lines {
        ...
        if line.contains("Some tests failed") || line.contains(" test failed") {
            summary = Some(line.clone());
        }
        if line.contains(" -") || line.contains("[E]") || line.contains(" Error:") {
            failures.push(line.clone());
        } else if line.to_lowercase().contains("exception") || line.to_lowercase().contains("error")
        {
