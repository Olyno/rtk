...
use crate::core::{runner, tee, toml_filter, tracking, utils};
use anyhow::Result;
use std::process::{Command, Stdio};

pub fn run(args: &[String], verbose: u8) -> Result<i32> {
    if has_json_reporter(args) {
        return run_passthrough(args, verbose);
    }
    if let Some(result) = run_with_toml_filter("flutter", args, verbose) {
        return result;
    }
...
/// Returns true if the command args request a machine-readable JSON test reporter.
/// JSON output must never be line-filtered or summarized as text.
pub(crate) fn has_json_reporter(args: &[String]) -> bool {
    for (i, arg) in args.iter().enumerate() {
        let lower = arg.to_lowercase();
        if lower == "--reporter=json"
            || (lower.starts_with("--reporter=") && lower.contains("json"))
        {
            return true;
        }
        if lower == "--reporter" {
            if let Some(next) = args.get(i + 1) {
                if next.to_lowercase().contains("json") {
                    return true;
                }
            }
        }
...
/// Run a Dart/Flutter invocation through the TOML filter pipeline when a filter
/// matches. Returns `None` when no filter applies so the caller can fall back
/// to its own summarizer or passthrough.
pub(crate) fn run_with_toml_filter(
    tool: &str,
    args: &[String],
    verbose: u8,
) -> Option<Result<i32>> {
    // Machine-readable JSON reporters must not be line-filtered.
    if has_json_reporter(args) {
        return None;
    }

    let full_command = std::iter::once(tool)
        .chain(args.iter().map(|s| s.as_str()))
        .collect::<Vec<_>>()
        .join(" ");
    let filter = toml_filter::find_matching_filter(&full_command)?;
    ...
    match result {
        Ok(output) => {
            let exit_code = utils::exit_code_from_output(&output, &full_command);
            ...
            let tee_hint = if !output.status.success() {
                tee::tee_and_hint(&combined_raw, &full_command, exit_code)
            } else {
                None
            };
            let filtered = toml_filter::apply_filter(filter, &combined_raw);
            let shown = runner::emit_guarded(&filtered, tee_hint.as_deref(), &combined_raw);
            timer.track(
                &full_command,
                &format!("rtk:toml {}", full_command),
                &combined_raw,
                &shown,
            );
            Some(Ok(exit_code))
