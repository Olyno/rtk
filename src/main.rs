...
/// Read mode for `rtk read --mode <mode>`.
#[derive(Debug, Clone, Copy, PartialEq, ValueEnum)]
pub enum ReadMode {
    /// Full file content (default)
    Full,
    /// Function/method/class signatures only
    Signatures,
    /// Signatures + imports (structural overview)
    Map,
}
...
enum Commands {
    ...
    /// Retrieve full archived output from a previous filtered command
    Expand {
        /// Archive ID to retrieve (shown in [rtk: full output archived, rtk expand <id> to view])
        #[arg(required = true)]
        id: String,
    },
    ...
    Read {
        ...
        /// Read mode: full (default), signatures (functions/classes only), map (signatures + imports)
    ...
    UninstallStandard: FnOnce(bool, bool, bool, bool, bool, bool, hooks::init::InitContext) -> Result<()>,
{
    if agent == Some(AgentTarget::Hermes) {
        ...
        uninstall_standard(global, gemini, codex, cursor, kimi, pi, ctx)
    }
}
...
fn run_cli() -> Result<i32> {
    ...
    let code = match cli.command {
        ...
        Commands::Expand { id } => {
            match crate::core::archive::retrieve(&id) {
                Some(entry) => {
                    println!("Command: {}", entry.command);
                    println!("Size: {} bytes", entry.raw_size);
                    println!("═══════════════════════════════════════");
                    println!("{}", entry.raw_output);
                }
                None => {
                    eprintln!("rtk: archive entry '{}' not found or expired", id);
                    return Ok(1);
                }
            }
            0
